...
public:
	Real *cent;
	cluster(unsigned nClusters, unsigned iterations, AllEmbeds& vecEmbeddings, bool UseEuclid = false) :
	...
	void Kmeans() {
		...
		for (a = 0; a < numIterations; a++) {
			...
			//cerr << a << endl;
			centroid = new Real*[numClusters];
			for (int i = 0; i < numClusters; i++) {
				centroid[i] = new Real[layer1_size];
				for (int j = 0; j < layer1_size; j++)
					centroid[i][j] = cent[i * layer1_size + j];
			}
		}
		...
		for (a = 0; a < numIterations; a++) {
			...
			for (c = 0; c < vec_size; c++) {
				...
				for (d = 0; d < numClusters; d++) {
					...
					for (b = 0; b < layer1_size; b++)
						x +=	(cent[layer1_size * d + b] - vectors[c][b])
							*	(cent[layer1_size * d + b] - vectors[c][b]);
					if (x < closev) {
						closev = x;
						closeid = d;
					}
