...
#define MAX_SIG_SIZE1  (MAX_SIG_SIZE+10)
/*****************************************************************************/
...
private:
	 ...
	 ******************************************************************************/
public:
	float weighted_emd(signature_t *Signature1, signature_t *Signature2, signature_t *weight, float **cost,
		...
		w = init(Signature1, Signature2, weight, cost);
		...
		for (XP = _X; XP < _EndX; XP++) {
			...
			if (XP->val == 0) /* ZERO FLOW */
				continue;
			///Use Weight to update the total cost
			totalCost += (float) XP->val * _C[XP -> i][XP -> j] * Weight[XP -> j];
			if (Flow != NULL) {
				FlowP->from = XP->i;
				FlowP->to = XP->j;
				FlowP->amount = XP->val;
...
private:
	/**********************
	 init
	 **********************/
	float init(signature_t *Signature1, signature_t *Signature2, signature_t *weight, float **cost) {
		int i, j;
				...
				for (CurV = v1Head.Next; CurV != NULL; CurV = CurV->Next) {
					...
					for (CurU = u0Head.Next; CurU != NULL; CurU = CurU->Next) {
						...
						if (_IsX[i][j]) {
							...
							CurU->Next =
									u1Head.Next != NULL ? u1Head.Next : NULL;
							u1Head.Next = CurU;
							CurU = PrevU;
						} else
							PrevU = CurU;
				...
				for (CurU = u1Head.Next; CurU != NULL; CurU = CurU->Next) {
					...
					for (CurV = v0Head.Next; CurV != NULL; CurV = CurV->Next) {
						...
						if (_IsX[i][j]) {
							...
							PrevV->Next = CurV->Next;
							CurV->Next =
									v1Head.Next != NULL ? v1Head.Next : NULL;
							v1Head.Next = CurV;
							CurV = PrevV;
						} else
							PrevV = CurV;
			...
			if (Remember == PrevUMinI->Next) /* LINE minI WAS DELETED */
			{
				for (CurV = vHead.Next; CurV != NULL; CurV = CurV->Next) {
					...
					if (CurV->val == _C[minI][j]) /* COLUMN j NEEDS UPDATING */
					{
						...
						oldVal = CurV->val;
						CurV->val = -INFINITY;
						for (CurU = uHead.Next; CurU != NULL; CurU =
								CurU->Next) {
							int i;
							i = CurU->i;
							if (CurV->val <= _C[i][j])
			...
			} else /* COLUMN minJ WAS DELETED */
			{
				for (CurU = uHead.Next; CurU != NULL; CurU = CurU->Next) {
					...
					if (CurU->val == _C[i][minJ]) /* ROW i NEEDS UPDATING */
					{
						...
						oldVal = CurU->val;
						CurU->val = -INFINITY;
						for (CurV = vHead.Next; CurV != NULL; CurV =
								CurV->Next) {
							int j;
							j = CurV->i;
							if (CurU->val <= _C[i][j])
